...
def debug_save_images(envs):
    if envs.num_envs_int == 1:
        ...
        save_image(envs.rgb_camera_handler.segmented_images[0][0].float().permute(2, 0, 1),
                    "./runs/sim_images_segment/latest_segmented.jpg")
    if envs.use_segmentation_for_inference and envs.num_envs_int == 1:
        os.makedirs("./runs/sim_images_rgb", exist_ok=True)
        save_image(envs.rgb_camera_handler.rgb_images[..., :3].permute(2, 0, 1) / 255.0,
                    "./runs/sim_images_rgb/latest_rgb.jpg")
                    
        os.makedirs("./runs/sim_images_sam2", exist_ok=True)
        save_image(envs.info["image"][0] * 255.0,
                    "./runs/sim_images_sam2/latest_sam2.jpg")
...
def print_step_debug(envs,):
    ...
    car_pos = envs.car_handler.pose_value[0, 4:7].detach().float().cpu()
    ...
    jaw_car_force_value = envs.jaw_car_sensor_handler.force_sensor_val[0].detach().float().cpu()
    gripper_car_force_value = envs.gripper_car_sensor_handler.force_sensor_val[0].detach().float().cpu()
    ...
    print(f"Car pos (xyz): {rlist(car_pos.tolist())}")
    ...
    print(f"Jaw->Car forces ({jaw_car_force_value.numel()}): {rlist(jaw_car_force_value.tolist())}")
    print(f"Gripper->Car forces ({gripper_car_force_value.numel()}): {rlist(gripper_car_force_value.tolist())}")
...
def update_draw_targets(envs):
    ...
    car_pose_value = envs.car_handler.pose_value.cpu()
    for i in range(envs.num_envs_int):
        rot = v.Quat(*car_pose_value[i][:4])
        pos = v.Vec3(*car_pose_value[i][4:])
        tf = v.Transform(rot, pos)

        envs.reach_space_targets[i].set_transform(tf)

        update_local_frame_lines(car_pose_value[i], envs.reach_axis_lines[i])
