...
def allocate_dynamic_state_gpu_buffers(envs):
    ...
    set_kine_cmd = envs.env_group.create_articulation_kinematic_state_command(
        v.wrap_gpu_buffer(envs.set_dof_pos_buf), v.wrap_gpu_buffer(envs.set_dof_vel_buf),
        v.wrap_gpu_buffer(envs.gpu_init_root_transforms),
        v.wrap_gpu_buffer(envs.gpu_init_root_velocities),
        envs.arti_handle, link_index_range=(0, 1),
        masks_buffer=v.wrap_gpu_buffer(envs.reset_buf))
    envs.gpu_set_kinematic_state_command_array = \
        envs.gym.create_articulation_kinematic_state_command_gpu_array([set_kine_cmd])

    # Get kinematic state GPU command
    get_kine_cmd = envs.env_group.create_articulation_kinematic_state_command(
        v.wrap_gpu_buffer(envs.get_dof_pos_buf), v.wrap_gpu_buffer(envs.get_dof_vel_buf),
        v.wrap_gpu_buffer(envs.root_pos_buf), v.wrap_gpu_buffer(envs.root_vel_buf),
        envs.arti_handle,
        link_index_range=(0, 1))
    envs.gpu_get_kinematic_state_command_array = \
        envs.gym.create_articulation_kinematic_state_command_gpu_array([get_kine_cmd])
...
def allocate_static_parameters_gpu_buffers(envs):
    ...
    # Link dynamic friction buffer
    envs.link_friction_buf = torch.zeros((num_env_sets,), device=envs.device, dtype=torch.float32)

    # Set link friction command
    set_link_friction_cmd = envs.env_group.create_rigid_material_property_command(
        v.RigidMaterialProperty.DYNAMIC_FRICTION,
        v.wrap_gpu_buffer(envs.link_friction_buf), envs.rigid_material_handle,
        masks_buffer=v.wrap_gpu_buffer(envs.reset_buf))
    envs.gpu_set_link_friction_command_array = \
        envs.gym.create_rigid_material_property_command_gpu_array([set_link_friction_cmd])


class AntEnvironmentRigidMaterialDomainRandomization(EnvironmentGpu):
    ...
    def __init__(self,
                 ...
                 randomize_friction_range: tuple[float, float] = (0.05, 1.0),
        ...
        self.randomize_friction_range = randomize_friction_range
    ...
    def create_envs(self):
        ...
        self.rigid_material_handle = env_def.create_rigid_material(material)

        self.art_def = env_def.get_articulation_def(self.art_def_handle)

        for i in range(self.art_def.get_num_link_defs()):
            env_def.assign_rigid_material_to_articulation_link(self.art_def_handle,
                                                               self.rigid_material_handle, i)
    ...
    def store_initial_conditions(self):
        ...
        material = env_def.get_rigid_material(self.rigid_material_handle)

        self.num_rigid_materials = env_def.get_num_rigid_materials()

        self.link_friction_low = torch.tensor([self.randomize_friction_range[0]],
                                              device=self.device, dtype=torch.float32)
        self.link_friction_high = torch.tensor([self.randomize_friction_range[1]],
                                               device=self.device, dtype=torch.float32)
    ...
    def reset_idx(self):
        ...
        r = torch.rand(len(self.num_envs), device=self.device, dtype=torch.float32)
        self.link_friction_buf[:] = (self.link_friction_high - self.link_friction_low) * r + \
            self.link_friction_low

        self.gym.set_rigid_material_properties(self.gpu_set_link_friction_command_array)
...
if __name__ == "__main__":
    ...
    from vlearn.utils import str_to_env_args
    ...
    d = dict()
    if len(argv) >= 3:
        d.update(str_to_env_args(AntEnvironmentRigidMaterialDomainRandomization, argv[2]))

    num_envs = 64

    randomize_friction_range = (0.05, 1.0)
    ...
    kwargs = dict(reset_noise_scale=reset_noise_scale, spacing=spacing,
                  ...
                  randomize_friction_range=randomize_friction_range, with_window=with_window)
    kwargs.update(d)

    envs = AntEnvironmentRigidMaterialDomainRandomization(num_envs, device, **kwargs)
    ...
    if render is not None:
        ...
        def control_by_menu():
            ...
            actions = torch.tensor([slider.get_value() for slider in sliders], dtype=torch.float32,
                                   device=device)
