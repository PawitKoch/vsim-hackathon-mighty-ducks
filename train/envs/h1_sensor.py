...
filename = "assets/vsim/H1/h1.vsim"
env_def.import_definitions(filename, True)
def_handle = env_def.get_articulation_def_handle_by_name("pelvis")
print("env_def.get_articulation_def(def_handle): {}".format(env_def.get_articulation_def(def_handle)))

rot = v.Quat(v.Vec3(1.0, 0.0, 0.0), 0)
pos = v.Vec3(0, 0, 0)
transform = v.Transform(rot, pos)

env_def.create_articulation(def_handle, transform, 'runner')
