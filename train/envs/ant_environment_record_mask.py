...
class AntEnvironmentRecordMask(EnvironmentGpu):
    ...
    def __init__(self,
                 ...
                 record_mask_buf: torch.Tensor = None
                 ):

        self.record_mask_buf = record_mask_buf
    ...
    def post_physics_step(self):
        ...
        if self.record_mask_buf is not None:
            self.set_record_mask()

    def set_record_mask(self):

        assert self.record_mask_buf is not None

        # Record top fastest ant
        x_new = self.root_pos_buf[:, 4]
        x_old = self.old_root_pos_buf[:, 4]
        vel = x_new - x_old
        index = torch.argmax(vel)

        self.record_mask_buf[:] = False
        self.record_mask_buf[index] = True

        # Record environments with NaNs in observations buffer
        self.record_mask_buf[:] = torch.where(torch.isnan(self.obs_buf).any(dim=1), True,
                                              self.record_mask_buf)
