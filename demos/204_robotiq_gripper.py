...
# Import gripper
filename = "assets/mjcf/robotiq_2f85/2f85.xml"
env_def.import_definitions(filename, fixed=True)

art_def_handle = env_def.get_articulation_def_handle_by_name("base_mount")
art_def = env_def.get_articulation_def(art_def_handle)

# Assign custom elastic material
mat = v.ElasticMaterial()

mat.stiffness = 0.5
mat.damping = 0.02
mat.limit_stiffness = 0.0
mat.limit_damping = 0.0

mat_handle = env_def.create_elastic_material(mat)

print("> Fixed tendon definitions:")
for tendon_def in art_def.get_fixed_tendon_defs():
    tendon_def.assign_elastic_material(mat_handle)
    print(tendon_def)
...
env_group.tile_environments(spacing=0.2)
...
art_def = env_def.get_articulation_def(art_def_handle)
num_tendons = art_def.get_num_fixed_tendon_defs()
...
# Create command
set_offset_cmd = env_group.create_fixed_tendon_control_command(
    v.wrap_gpu_buffer(set_offset_buf),
    art_handle,
    (0, num_tendons))

set_offset_cmd_arr = gym.create_fixed_tendon_control_command_gpu_array([set_offset_cmd])

# == Set up render == #
render = gym.get_render()

render.reset_camera(v.Vec3(0.1, 0.2, 0.6), v.Vec3(0, -0.2, -1))
...
def set_control_offsets():
    ...
    # Set control offset
    gym.set_fixed_tendon_controls(set_offset_cmd_arr)
