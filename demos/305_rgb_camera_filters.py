...
# == Applying filters to raw RGB image data == #
# Create command
...
mean = 0
std = 0.1
gaussian_noise_cmd = env_group.create_gaussian_noise_rgb_command(camera_handle, mean, std)
gaussian_noise_cmd_arr = gym.create_gaussian_noise_rgb_command_gpu_array([gaussian_noise_cmd])

# Erase pixels
percent = 0.05
erase_pixels_cmd = env_group.create_erase_pixels_rgb_command(camera_handle, percent)
erase_pixels_cmd_arr = gym.create_erase_pixels_rgb_command_gpu_array([erase_pixels_cmd])

# Erase patch
size_x = 10
size_y = 5
erase_patch_cmd = env_group.create_erase_patch_rgb_command(camera_handle, size_x, size_y)
erase_patch_cmd_arr = gym.create_erase_patch_rgb_command_gpu_array([erase_patch_cmd])
...
def grab_image():
    ...
    plt.savefig("305_rgb_camera_filters.png")
    print("Image saved in 305_rgb_camera_filters.png")
...
def apply_filter():
    ...
    if filter_box_0.get_value():
        gym.apply_rgb_camera_gaussian_noise(gaussian_noise_cmd_arr)

    if filter_box_1.get_value():
        gym.apply_rgb_camera_erase_pixels(erase_pixels_cmd_arr)

    if filter_box_2.get_value():
        gym.apply_rgb_camera_erase_patch(erase_patch_cmd_arr)
