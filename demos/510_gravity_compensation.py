...
# Write into same buffer as set_force_buf
compute_gravity_cmd = env_group.create_gravity_compensation_command(
    v.wrap_gpu_buffer(set_force_buf),
    art_handle,
    gym.get_gravity())

compute_gravity_cmd_arr = gym.create_gravity_compensation_command_gpu_array([
    compute_gravity_cmd])
...
# == GUI controls == #
gravity_compensation_box = v.UserCheckbox("Gravity compensation", True)

render.register_menu_item(gravity_compensation_box)
...
while not done:
    ...
    gym.compute_gravity_compensations(compute_gravity_cmd_arr)
    if not gravity_compensation_box.get_value():
        set_force_buf[:] = 0
