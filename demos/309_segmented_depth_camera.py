...
# == Getting segmented depth camera image == #
# Segmented depth image buffer: shape is (num_envs, resolution_y, resolution_x)
...
get_seg_cmd = env_group.create_segmented_depth_camera_command(v.wrap_gpu_buffer(get_seg_buf),
                                                              camera_handle, seg_handles)

get_seg_cmd_arr = gym.create_segmented_depth_camera_command_gpu_array([get_seg_cmd])
...
def grab_image():
    ...
    gym.get_segmented_depth_camera_images(get_seg_cmd_arr)

    plt.imshow(get_seg_buf[0].cpu())
    plt.gca().invert_yaxis()
    plt.colorbar()

    plt.savefig("309_segmented_depth_camera.png")
    print("Image saved in 309_segmented_depth_camera.png")
